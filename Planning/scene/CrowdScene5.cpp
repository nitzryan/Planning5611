...
CrowdScene5::CrowdScene5() :
	...
	map(Material(ColorRGBA(0.8f, 0.8f, 0.8f, 1.0f), 1.0f, 0.0f, 0.0f, 10.0f, -1), mt, {
		...
		CrowdDest(-4.0f, -1.0f, 4.0f, 1.0f, destMat, Pos2F(-1.2f, 2.5f), Pos2F(-0.8f, 2.5f)),
		CrowdDest(1.0f, 4.0f, 4.0f, 1.0f, destMat, Pos2F(1.2f, 2.5f), Pos2F(0.8f, 2.5f)),
		CrowdDest(-4.0f, -1.0f, -1.0f, -4.0f, destMat, Pos2F(-1.2f, -2.5f), Pos2F(-0.8f, -2.5f)),
		CrowdDest(1.0f, 4.0f, -1.0f, -4.0f, destMat, Pos2F(1.2f, -2.5f), Pos2F(0.8f, -2.5f)),
		})
{
...
void CrowdScene5::Render(Renderer& renderer)
{
	for (auto& i : people) {
		if (!i.ReadyToRecycle()) {
			renderer.Render(i);
...
void CrowdScene5::Update(float dt)
{
	...
	// Avoid getting pushed in the wrong direction
	for (auto& i : people) {
		i.AvoidWrongDirectionMovement(dt);
	}

	// Adjust movement to avoid moving into another character
	for (auto& i : people) {
		for (auto& j : people) {
			if (&i != &j) { // DifferentPeople
				i.AvoidImminentCollision(j, dt);
			}
		}
	}
